...
TypeId Ipv4L3Protocol::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::Ipv4L3Protocol").SetParent<Ipv4>().AddConstructor<
					Ipv4L3Protocol>().AddAttribute("DefaultTos",
					...
					MakeUintegerChecker<uint8_t>()).AddAttribute("DefaultTtl",
					...
					MakeTraceSourceAccessor(&Ipv4L3Protocol::m_txTrace)).AddTraceSource(
					"Rx", "Receive ipv4 packet from incoming interface.",
					MakeTraceSourceAccessor(&Ipv4L3Protocol::m_rxTrace)).AddTraceSource(
					"Drop", "Drop ipv4 packet",
					MakeTraceSourceAccessor(&Ipv4L3Protocol::m_dropTrace)).AddAttribute(
					"InterfaceList",
			...
			.AddTraceSource("SendOutgoing",
					...
					MakeTraceSourceAccessor(
							&Ipv4L3Protocol::m_sendOutgoingTrace)).AddTraceSource(
					"UnicastForward",
					"A unicast IPv4 packet was received by this node and is being forwarded to another node",
					MakeTraceSourceAccessor(
							&Ipv4L3Protocol::m_unicastForwardTrace)).AddTraceSource(
					"LocalDeliver",
...
Ptr<IpL4Protocol> Ipv4L3Protocol::GetProtocol(int protocolNumber) const {
	...
	for (L4List_t::const_iterator i = m_protocols.begin();
			i != m_protocols.end(); ++i) {
...
void Ipv4L3Protocol::SendRealOut(Ptr<Ipv4Route> route, Ptr<Packet> packet,
	...
	/**--------------------Chunzhi------------------------**/
	bool nodeIsServer = this->GetNInterfaces() == 2;
	...
	if (!route->GetGateway().IsEqual(Ipv4Address("0.0.0.0"))) {
		if (outInterface->IsUp()) {
			...
			if (packet->GetSize() > outInterface->GetDevice()->GetMtu()) {
				...
				DoFragmentation(packet, outInterface->GetDevice()->GetMtu(),
						listFragments);
				for (std::list<Ptr<Packet> >::iterator it =
...
void Ipv4L3Protocol::LocalDeliver(Ptr<const Packet> packet, Ipv4Header const&ip,
	...
	if (protocol != 0) {
		...
		enum IpL4Protocol::RxStatus status = protocol->Receive(p, ip,
				GetInterface(iif));
		switch (status) {
		...
		case IpL4Protocol::RX_ENDPOINT_UNREACH:
			...
			for (uint32_t i = 0; i < GetNAddresses(iif); i++) {
				...
				if (addr.GetLocal().CombineMask(addr.GetMask())
						...
						&& ip.GetDestination().IsSubnetDirectedBroadcast(
								addr.GetMask())) {
					subnetDirected = true;
				}
			}
...
void Ipv4L3Protocol::DoFragmentation(Ptr<Packet> packet, uint32_t outIfaceMtu,
		std::list<Ptr<Packet> >& listFragments) {
