...
TypeId Ipv4GlobalRouting::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::Ipv4GlobalRouting").SetParent<Object>().AddAttribute(
					"RandomEcmpRouting",
					"Set to true if packets are randomly routed among ECMP; set to false for using only one route consistently",
					BooleanValue(false),
					MakeBooleanAccessor(
							&Ipv4GlobalRouting::m_randomEcmpRouting),
					...
					BooleanValue(false),
					MakeBooleanAccessor(
							&Ipv4GlobalRouting::m_respondToInterfaceEvents),
					MakeBooleanChecker());
	return tid;
}

Ipv4GlobalRouting::Ipv4GlobalRouting() :
		m_randomEcmpRouting(false), m_respondToInterfaceEvents(false) {
...
void Ipv4GlobalRouting::AddHostRouteTo(Ipv4Address dest, uint32_t interface) {
...
void Ipv4GlobalRouting::AddNetworkRouteTo(Ipv4Address network,
		Ipv4Mask networkMask, Ipv4Address nextHop, uint32_t interface) {
...
void Ipv4GlobalRouting::AddNetworkRouteTo(Ipv4Address network,
		Ipv4Mask networkMask, uint32_t interface) {
...
void Ipv4GlobalRouting::AddASExternalRouteTo(Ipv4Address network,
		Ipv4Mask networkMask, Ipv4Address nextHop, uint32_t interface) {
	NS_LOG_FUNCTION (this << network << networkMask << nextHop << interface);
	Ipv4RoutingTableEntry *route = new Ipv4RoutingTableEntry();
	*route = Ipv4RoutingTableEntry::CreateNetworkRouteTo(network, networkMask,
...
Ptr<Ipv4Route> Ipv4GlobalRouting::LookupGlobal(Ipv4Address dest,
	...
	if (allRoutes.size() > 0) // if route(s) is found
		...
		rtentry->SetSource(
				m_ipv4->GetAddress(route->GetInterface(), 0).GetLocal());
...
uint32_t Ipv4GlobalRouting::GetNRoutes(void) const {
	NS_LOG_FUNCTION (this);
	uint32_t n = 0;
	n += m_hostRoutes.size();
...
Ipv4GlobalRouting::GetRoute(uint32_t index) const {
	...
	if (index < m_hostRoutes.size()) {
		uint32_t tmp = 0;
		for (HostRoutesCI i = m_hostRoutes.begin(); i != m_hostRoutes.end();
				i++) {
			if (tmp == index) {
				return *i;
			}
...
void Ipv4GlobalRouting::RemoveRoute(uint32_t index) {
	NS_LOG_FUNCTION (this << index);
	if (index < m_hostRoutes.size()) {
		uint32_t tmp = 0;
		for (HostRoutesI i = m_hostRoutes.begin(); i != m_hostRoutes.end();
				i++) {
			if (tmp == index) {
...
int64_t Ipv4GlobalRouting::AssignStreams(int64_t stream) {
	NS_LOG_FUNCTION (this << stream);
	m_rand->SetStream(stream);
	return 1;
}

void Ipv4GlobalRouting::DoDispose(void) {
	NS_LOG_FUNCTION (this);
	...
	for (ASExternalRoutesI l = m_ASexternalRoutes.begin();
			l != m_ASexternalRoutes.end(); l = m_ASexternalRoutes.erase(l)) {
		delete (*l);
	}
...
void Ipv4GlobalRouting::PrintRoutingTable(
	...
	if (GetNRoutes() > 0) {
		*os
		...
		for (uint32_t j = 0; j < GetNRoutes(); j++) {
			...
			dest << route.GetDest();
			*os << std::setiosflags(std::ios::left) << std::setw(16)
					<< dest.str();
			gw << route.GetGateway();
			*os << std::setiosflags(std::ios::left) << std::setw(16)
					<< gw.str();
			...
			*os << std::setiosflags(std::ios::left) << std::setw(6)
					<< flags.str();
			...
			*os << "-" << "   ";
			if (Names::FindName(m_ipv4->GetNetDevice(route.GetInterface()))
				...
				*os
						<< Names::FindName(
								m_ipv4->GetNetDevice(route.GetInterface()));
			} else {
...
Ptr<Ipv4Route> Ipv4GlobalRouting::RouteOutput(Ptr<Packet> p,
...
bool Ipv4GlobalRouting::RouteInput(Ptr<const Packet> p,
...
void Ipv4GlobalRouting::SetIpv4(Ptr<Ipv4> ipv4) {
