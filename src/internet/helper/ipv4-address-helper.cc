...
Ipv4InterfaceContainer
Ipv4AddressHelper::Assign_FatTree (NetDeviceContainer &c, uint32_t ip1_, uint32_t ip2_, char const *mask_)
{
  NS_LOG_FUNCTION_NOARGS ();
  Ipv4InterfaceContainer retval;
  for (uint32_t i = 0; i < 2; ++i) {
      ...
      if (i ==0)
      {
    	  Ipv4InterfaceAddress ipv4Addr = Ipv4InterfaceAddress (Ipv4Address(ip1_), Ipv4Mask(mask_));
    	  ipv4->AddAddress (interface, ipv4Addr);
    	  ipv4->SetMetric (interface, 1);
    	  ipv4->SetUp (interface);
    	  retval.Add (ipv4, interface);
      ...
      {
    	  Ipv4InterfaceAddress ipv4Addr = Ipv4InterfaceAddress (Ipv4Address(ip2_), Ipv4Mask(mask_));
    	  ipv4->AddAddress (interface, ipv4Addr);
    	  ipv4->SetMetric (interface, 1);
    	  ipv4->SetUp (interface);
    	  retval.Add (ipv4, interface);
