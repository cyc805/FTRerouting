...
TypeId BulkSendApplication::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::BulkSendApplication").SetParent<Application>().AddConstructor<
					BulkSendApplication>().AddAttribute("SendSize",
					...
					MakeTraceSourceAccessor(&BulkSendApplication::m_txTrace)).AddAttribute(
					"TraceCwndOutputFile",
					"output file for tracing congestiong window. Disable tracing when = empty ",
					StringValue(""),
					MakeStringAccessor(&BulkSendApplication::m_cwndOutputFile),
					MakeStringChecker());
	return tid;
}
...
Ptr<Socket> BulkSendApplication::GetSocket(void) const {
	NS_LOG_FUNCTION (this);
	return m_socket;
}
...
void BulkSendApplication::ConnectionSucceeded(Ptr<Socket> socket) {
	NS_LOG_FUNCTION (this << socket);NS_LOG_LOGIC ("BulkSendApplication Connection succeeded");
	m_connected = true;
	SendData();
}

void BulkSendApplication::ConnectionFailed(Ptr<Socket> socket) {
	NS_LOG_FUNCTION (this << socket);NS_LOG_LOGIC ("BulkSendApplication, Connection Failed");
}
